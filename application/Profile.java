...
public class Profile {
    ...
    public static void deleteProfile(String name) throws IOException {
        ...
        for (GameState elem : saves) {
            fileWriter3.write(elem.getName() + "," + elem.getCurrLevel() + "," + elem.getTime() + "," + elem.getItems() + "\n");
        }
        fileWriter3.close();
    }
     ...
     * @throws IOException
     */
    public static void saveGame(String name, int currLevel, String time, String map, String items) throws IOException {
        saves = GameReader.readGame("GameFile.txt");

        GameState gamestate = new GameState(name, currLevel, time, map, items);
        ...
        if (exist == false) {
            saves.add(gamestate);
            for (GameState elem : saves) {
                fileWriter.write(elem.getName() + "," + elem.getCurrLevel() + "," + elem.getTime() + "," + elem.getItems() + "\n");
            }
        } else {
            saves.remove(remove);
            saves.add(gamestate);
            fileWriter2.write(gamestate.getName() + "," + gamestate.getCurrLevel() + "," + gamestate.getTime() + "," + gamestate.getItems() + "\n");
